...
- 실제로 컨테이너 생명주기(생성, 실행, 삭제)를 관리하는 메인 엔진

### runc
- OCI(Open Container Initiative) 표준을 구현한 저수준 런타임.
- containerd 가 직접 컨테이너를 실행하지 않고, runc를 호출해서 프로세스를 격리(cgroups, namespaces)시킴.

### CNI Plugin
- 컨테이너 네트워크 세팅 담당.
- kubelet 이 Pod 만들 때 containerd 에게 요청 → containerd 가 CNI 플러그인 호출 → Pod 의 veth 인터페이스, 브릿지, IP 할당 설정.
- Flannel, Calico, Cilium 같은 네트워크 솔루션도 사실은 CNI 플러그인 기반.
